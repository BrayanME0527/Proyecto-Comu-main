...
def transmisor(x_t):
    ...
    #Su codigo para el transmisor va aca ("completar")
...
def canal(s_t):
    ...
    #Su codigo para el canal va aca. ("completar")
    
    #s_t_prima=s_t #eliminar cuando se tenga solucion propuesta

    # Aplicar ruido gaussiano al señal transmitida
    ruido = np.random.normal(mu, sigma, len(s_t))
    s_t_prima = s_t + ruido
...
def receptor(s_t_prima,f_rf):
    ...
    #Su codigo para el receptor va aca  ("completar")
